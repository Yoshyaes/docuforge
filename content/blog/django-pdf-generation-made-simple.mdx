...
description: "Add PDF generation to your Django app with the Deckle Python SDK. Build views for HTML-to-PDF, template-based invoices, and file downloads."
...
Deckle eliminates this complexity. One `pip install`, one API call, and you get back a pixel-perfect PDF rendered by a headless Chromium instance managed entirely by the Deckle service. The Python SDK is synchronous and fits naturally into Django's request-response cycle.
...
Start by installing the Deckle Python SDK alongside your Django project dependencies.

```bash
pip install deckle
```

Add your API key to your Django settings. You can grab one from the [Deckle dashboard](https://app.getdeckle.dev/keys). Never hard-code API keys in source files -- pull them from environment variables.
...
def get_deckle_client():
    return Deckle(api_key=settings.DECKLE_API_KEY)
```
...
from .utils import get_deckle_client
...
The `@csrf_exempt` decorator is necessary here because this endpoint receives JSON from API clients rather than Django form submissions. The `@require_POST` decorator rejects anything that is not a POST request. The Deckle error hierarchy maps cleanly to HTTP status codes: `AuthenticationError` to 401, `RateLimitError` to 429, `ValidationError` to 400, and the base `DeckleError` to 502 for upstream failures.

## Step 3: Template-Based Invoice Download

Most production use cases involve templates rather than raw HTML. This view fetches an invoice from your database, passes its data to a Deckle template, and returns the PDF as a direct file download.
...
def download_invoice_pdf(request, invoice_id):
    ...
    with Deckle(api_key=settings.DECKLE_API_KEY) as df:
...
from django.conf import settings
from deckle import Deckle, DeckleError
...
Using the mixin, any view that already builds a context dictionary can add a PDF export route with minimal code. The `render_to_string` call renders a Django template to HTML, which Deckle then converts to PDF. This lets you reuse your existing Django templates -- the same Jinja-style markup you already write for web pages can produce PDFs with no modifications.
...
This tutorial covered the core patterns for integrating Deckle into a Django application -- function-based views, template-based downloads, DRF integration, class-based views, and admin actions. From here you can explore several directions:

- **FastAPI integration** -- If parts of your system use FastAPI, the SDK works identically. See our [FastAPI tutorial](https://getdeckle.dev/blog/pdf-generation-python-fastapi-deckle) for async patterns and Pydantic model examples.
- **Comparing rendering approaches** -- If you are evaluating Deckle against Puppeteer or WeasyPrint, the [Deckle vs Puppeteer](https://getdeckle.dev/blog/deckle-vs-puppeteer-why-i-switched) article breaks down the trade-offs in performance, accuracy, and operational overhead.

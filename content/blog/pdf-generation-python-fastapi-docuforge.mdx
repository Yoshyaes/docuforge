...
Next, set your DocuForge API key as an environment variable. You can grab one from the [DocuForge dashboard](/keys).
...
- **Django integration** -- If your project uses Django rather than FastAPI, the SDK works identically. See our [Django tutorial](/blog/pdf-generation-django-docuforge) for middleware patterns and class-based view examples.
...
- **Page layout control** -- Our [page layout guide](/docs/page-layout) covers headers, footers, page numbers, and multi-column layouts in detail.
- **Comparison with local rendering** -- If you are evaluating DocuForge against Puppeteer or WeasyPrint, the [DocuForge vs Puppeteer](/blog/docuforge-vs-puppeteer) article breaks down the trade-offs in performance, accuracy, and operational overhead.

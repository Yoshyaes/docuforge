...
For **reporting**, look at our guide on [generating Supabase reports with DocuForge](/blog/supabase-reports-docuforge) -- the same template and batch patterns apply to monthly financial summaries.

If you want more control over **page layout** -- custom headers per page, landscape tables, or page break control -- the [page layout and headers guide](/blog/page-layout-headers-footers) covers all the options available in the `PDFOptions` object.

For teams building a full SaaS billing portal, the [Next.js integration guide](/docs/frameworks/nextjs) shows how to add a customer-facing invoice history page with on-demand PDF regeneration.

The complete source code from this tutorial is available in the [DocuForge examples repository](https://github.com/docuforge/examples/tree/main/stripe-invoices).

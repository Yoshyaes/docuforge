...
- **[Automate Stripe invoices](https://getdocuforge.dev/blog/invoice-system-stripe-docuforge)** -- Generate and email PDF invoices on every successful payment using Stripe webhooks.
- **[Export React components to PDF](https://getdocuforge.dev/blog/add-pdf-export-react-app)** -- Use `df.fromReact()` to render your existing React components directly to PDF without maintaining separate HTML templates.
- **[DocuForge vs Puppeteer](https://getdocuforge.dev/blog/docuforge-vs-puppeteer-why-i-switched)** -- Understand the tradeoffs between a managed API and self-hosted browser automation.
- **[Generate PDFs in Express](https://getdocuforge.dev/blog/express-pdf-api-html-to-pdf)** -- If your backend is not Next.js, the same SDK works in any Node.js environment.

Check out the [full API reference](https://fred-7da601c6.mintlify.app/api-reference) for advanced features like batch generation, webhook callbacks, and PDF tools including merge, split, and password protection.

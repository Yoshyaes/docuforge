...
- A **DocuForge account** and API key (grab one from the [dashboard](/dashboard))
- **Node.js 18** or later
...
- **[Automate Stripe invoices](/blog/automate-stripe-invoices-pdf)** -- Generate and email PDF invoices on every successful payment using Stripe webhooks.
- **[Export React components to PDF](/blog/react-component-to-pdf-export)** -- Use `df.fromReact()` to render your existing React components directly to PDF without maintaining separate HTML templates.
- **[DocuForge vs Puppeteer](/blog/docuforge-vs-puppeteer-pdf-generation)** -- Understand the tradeoffs between a managed API and self-hosted browser automation.
- **[Generate PDFs in Express](/blog/generate-pdfs-express-docuforge)** -- If your backend is not Next.js, the same SDK works in any Node.js environment.

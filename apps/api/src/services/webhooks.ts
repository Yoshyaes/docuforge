...
export async function deliverWebhook(
  ...
  if (!secret) {
    if (process.env.NODE_ENV === 'production') {
      console.error('WEBHOOK_SIGNING_SECRET is not set — skipping webhook delivery in production');
      return;
    }
    console.warn('WEBHOOK_SIGNING_SECRET is not set — using insecure dev default');
  }
  ...
  try {
    ...
    console.error(`Webhook URL rejected (SSRF protection): ${url}`, err);
    return;
  }
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      if (ok) return;
      console.warn(`Webhook delivery attempt ${attempt + 1} got non-2xx for ${url}`);
    } catch (err) {
      console.warn(`Webhook delivery attempt ${attempt + 1} failed for ${url}:`, err);
    }
  ...
  console.error(`Webhook delivery exhausted all retries for ${url}`);
}
